t_path = '../data/rawtext/kaiti/train'
augment_t_path = '../data/rawtext/augment/'
t_db_size = 750
augment_t_db_size = 7
save_GB = '../save/GB-erode-yaheiB.ckpt'
GB_nl = 6
DB_nl = 5
GB_nf = 32
DB_nf = 32
...
Btraining_num = 12800
print(f"""
--text_path {t_path} --augment_text_path {augment_t_path} --text_datasize {t_db_size} --augment_text_datasize {augment_t_db_size} --save_GB_name {save_GB} --GB_nlayers {GB_nl} --DB_nlayers {DB_nl} --GB_nf {GB_nf} --DB_nf {DB_nf} --epochs {epochs} --batchsize {batch_size} --Btraining_num {Btraining_num} --gpu
""")
